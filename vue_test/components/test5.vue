<template lang="html">
    <div class="test5">
        <div class="bar">
            <a :class="['list-icon', {active: showType === 'list'}]"
                @click="changeType('list')"
            ></a>
            <a :class="['grid-icon', {active: showType === 'grid'}]"
                @click="changeType('grid')"
            ></a>
        </div>
        <ul :class="showType">
            <li v-for="(item, index) in configs">
                <a href="">
                    <img :src="item.imgSrc" alt="">
                </a>
                <p v-if="showType === 'list'">{{ item.text }}</p>
            </li>
        </ul>
    </div>
</template>
...
.test5 {
    .bar{
        ...
        background-image: gradient(top, #5c9bb7, #5392ad);
